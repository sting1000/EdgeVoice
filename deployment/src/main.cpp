...
void printUsage(const char* program_name) {
    ...
    std::cout << "  <模型路径>       - OMC模型文件的路径，默认为/data/local/tmp/edgevoice_fast.omc" << std::endl;
    std::cout << "  <音频文件路径>   - 要识别的WAV音频文件路径" << std::endl;
    std::cout << "  [音频文件路径2]  - 可选的其他WAV文件路径，用于批处理" << std::endl;
    std::cout << std::endl;
    std::cout << "示例:" << std::endl;
    std::cout << "  " << program_name << " /data/local/tmp/edgevoice_fast.omc test.wav" << std::endl;
    std::cout << "  " << program_name << " /data/local/tmp/edgevoice_fast.omc test1.wav test2.wav test3.wav" << std::endl;
}

int main(int argc, char* argv[]) {
    std::string model_path = "/data/local/tmp/edgevoice_fast.omc";  // 默认模型路径
    ...
    try {
        std::cout << "初始化EdgeVoice推理引擎..." << std::endl;
        ...
        std::cout << "已加载模型，开始处理音频..." << std::endl;
