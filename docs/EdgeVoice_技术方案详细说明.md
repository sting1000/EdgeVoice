...
1. **输入投影层**:
   - 参数数量: 39(输入维度) × 224(隐藏维度) + 224(偏置) ≈ 8,960

2. **位置编码层**:
   - 参数数量: 500(最大位置) × 224(隐藏维度) ≈ 112,000 (非训练参数)

3. **Conformer块** (3层):
   - 每层前馈网络1: 224 × 4 × 224 + 896 ≈ 201,824
   - 每层自注意力模块: 8(头) × 224 × 3 × 224/8 + 896 ≈ 151,872
   - 每层卷积模块: 224 × 224 × 2 + 31 × 224 + 448 + 224 × 2 ≈ 106,624
   - 每层前馈网络2: 同前馈网络1 ≈ 201,824
   - 每层层归一化: 224 × 2 ≈ 448
   - 每层总计: 约662,592
   - 3层总计: 约1,987,776

4. **分类层**:
   - 参数数量: 224(输入) × 8(输出) + 8(偏置) = 1,800
...
2. **激活内存**:
   - 序列表示: 500(帧) × 224(隐藏维度) × 4(字节) ≈ 448KB
...
1. **资源需求增加**:
   - 与原CNN-GRU架构相比内存占用增加(~4MB vs ~350KB)
   - 计算量增加约3倍，但仍在可接受范围
