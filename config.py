...
# 特征提取参数（核心）
N_MFCC = 16  # MFCC系数数量，增加可提供更多频域细节，推荐范围：13-40
...
# INTENT_CLASSES = ["TAKE_PHOTO", "START_RECORDING", "CAPTURE_AND_DESCRIBE", "OTHERS"]  
...
CONFORMER_LAYERS = 6  # 层数，增加可提高模型容量，也增加计算量，推荐范围：2-6
CONFORMER_ATTENTION_HEADS = 8  # 注意力头数，通常为2的幂，推荐范围：4-16
CONFORMER_HIDDEN_SIZE = 128  # 隐藏层大小，影响模型容量，推荐为16的倍数（满足硬件优化）
CONFORMER_CONV_KERNEL_SIZE = 7  # 卷积核大小，影响感受野，推荐范围：9-31（奇数）
...
# 流式处理参数（核心，影响实时推理）
STREAMING_CHUNK_SIZE = 320  # 每次处理的帧数，减小可降低延迟但可能降低准确率，推荐范围：10-30
MAX_CACHED_FRAMES = 320  # 历史缓存帧数，增加可保留更多上下文，推荐范围：40-100
STREAMING_STEP_SIZE = 160  # 步长，影响特征重叠率，推荐范围：5-20
...
# 训练策略参数
LOSS_FUNC_CROSS_ENTROPY = 0 # 使用Cross Entropy损失函数
LOSS_FUNC_LABEL_SMOOTHING = 1  # 使用标签平滑
LOSS_FUNC_FOCAL_LOSS = 2 # 使用FOCAL Loss损失函数

LABEL_SMOOTHING_VALUE = 0.15  # 标签平滑强度，推荐范围：0.05-0.2
FOCAL_LOSS_ALPHA_VALUE = [] # Focal loss 正负样本比例调节参数维度与函数output一致
FOCAL_LOSS_GAMMA_VALUE = [] # Focal loss 聚焦参数gamma维度与函数output一致
USE_COSINE_SCHEDULER = True  # 是否使用余弦学习率调度
USE_EARLY_STOPPING = True  # 是否使用早停
EARLY_STOPPING_PATIENCE = 10  # 早停耐心值，推荐范围：5-10
