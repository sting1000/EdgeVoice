...
# Conformer模型参数 - 优化方案
CONFORMER_LAYERS = 3  # 增加层数，提高模型容量
CONFORMER_ATTENTION_HEADS = 8
CONFORMER_HIDDEN_SIZE = 160  # 修改为16的倍数，满足FP16 16通道对齐
CONFORMER_CONV_KERNEL_SIZE = 15  # 减小卷积核，适应短命令特性
CONFORMER_FF_EXPANSION_FACTOR = 4
CONFORMER_DROPOUT = 0.25  # 微调dropout
...
# 流式处理参数 - 优化后
STREAMING_CHUNK_SIZE = 20  # 减小每次处理的帧数，适应短命令
MAX_CACHED_FRAMES = 60    # 增加历史缓存，保留更多上下文
STREAMING_STEP_SIZE = 10    # 减小步长，增加特征重叠

# 数据增强参数
AUGMENT_PROB = 0.8  # 总体数据增强概率
MIXUP_ALPHA = 0.2   # MixUp增强强度
TIME_MASK_MAX_LEN = 8  # 时间掩码最大长度
FREQ_MASK_MAX_LEN = 5  # 频率掩码最大长度
